...
<body class="bg-gradient-to-br from-emerald-950 via-emerald-900 to-green-950 text-slate-100 min-h-screen antialiased selection:bg-amber-400 selection:text-emerald-950">
    ...
    <!-- Articles & Friday Khutbahs Section (Module 1) -->
    <section class="py-20 bg-gradient-to-br from-emerald-950 via-emerald-900 to-green-950">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-12">
                <span class="text-xs font-bold uppercase tracking-widest text-amber-400 bg-emerald-800/50 px-4 py-1.5 rounded-full border border-amber-500/10">KAJIAN & KHOTBAH</span>
                <h2 class="text-3xl sm:text-4xl font-extrabold text-white mt-4">Artikel Religi & Khotbah Jumat</h2>
                <p class="text-slate-300 text-sm mt-3 max-w-xl mx-auto">Perdalam tsaqafah Islamiyah melalui tulisan keagamaan dan rangkuman intisari Khotbah Jumat.</p>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($articles as $art)
                    <a href="{{ route('public.articles.show', $art->slug) }}"
                    class="block bg-gradient-to-b from-emerald-900/40 to-emerald-950 rounded-3xl overflow-hidden border border-emerald-850 shadow-xl group hover:border-amber-500/30 transition-all duration-300">
                        ...
                        <div class="p-6">
                            <div class="flex items-center justify-between mb-4">
                                <span class="px-3 py-1 rounded-full bg-emerald-800 text-[10px] uppercase font-bold text-amber-400 border border-amber-500/10">
                                    {{ $art->category === 'khotbah' ? 'Khotbah Jumat' : 'Artikel' }}
                                </span>
                            ...
                            <h3 class="text-lg font-bold text-white group-hover:text-amber-400 transition-colors mb-3">
                                {{ $art->title }}
                            </h3>

                            <p class="text-slate-300 text-sm line-clamp-4 leading-relaxed mb-4 whitespace-pre-line">
                                {{ $art->content }}
                            </p>
                        </div>
                ...
                @empty
                    <div class="col-span-full bg-emerald-900/10 border border-emerald-800 p-8 rounded-2xl text-center text-slate-400">
                        Belum ada artikel atau khotbah jumat yang dipublikasikan.
                    </div>
                @endforelse
            </div>
        </div>
