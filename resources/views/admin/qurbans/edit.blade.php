<x-app-layout>
    ...
    <div class="max-w-xl bg-white border border-gray-200 shadow-sm rounded-2xl p-8">
        <form action="{{ route('admin.qurbans.update', $qurban->id) }}" method="POST" class="space-y-6 text-xs" x-data="{ type: '{{ old('type', $qurban->type) }}' }">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <select id="year" name="year" required class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-850">
                        <option value="1447 H / 2026" {{ old('year', $qurban->year) === '1447 H / 2026' ? 'selected' : '' }}>1447 H / 2026</option>
                        <option value="1448 H / 2027" {{ old('year', $qurban->year) === '1448 H / 2027' ? 'selected' : '' }}>1448 H / 2027</option>
                        <option value="1446 H / 2025" {{ old('year', $qurban->year) === '1446 H / 2025' ? 'selected' : '' }}>1446 H / 2025</option>
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" id="participant_name" name="participant_name" required value="{{ old('participant_name', $qurban->participant_name) }}" placeholder="Nama Lengkap" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-805">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $qurban->phone) }}" placeholder="08xxxxxxxxxx" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-805">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2" x-show="type === 'sapi'">
                    ...
                    <input type="number" id="group_number" name="group_number" min="1" value="{{ old('group_number', $qurban->group_number) }}" placeholder="1" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800 font-mono font-semibold">
                ...
                <div class="space-y-2" :class="type === 'sapi' ? 'col-span-1' : 'col-span-2'">
                    ...
                    <input type="number" id="amount_paid" name="amount_paid" required min="0" value="{{ old('amount_paid', (int)$qurban->amount_paid) }}" placeholder="3500000" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800 font-mono font-semibold">
                ...
                <div class="space-y-2">
                    ...
                    <select id="status" name="status" required class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-850">
                        <option value="belum_lunas" {{ old('status', $qurban->status) === 'belum_lunas' ? 'selected' : '' }}>Belum Lunas</option>
                        <option value="lunas" {{ old('status', $qurban->status) === 'lunas' ? 'selected' : '' }}>Lunas</option>
                    </select>
            ...
            <div class="space-y-2">
                ...
                <textarea id="notes" name="notes" rows="3" placeholder="Misal: Atas nama almarhum, ukuran hewan, dsb..." class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800">{{ old('notes', $qurban->notes) }}</textarea>
