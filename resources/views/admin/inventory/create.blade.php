<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Tambah Aset Inventaris Baru') }}
            </h2>
    ...
    <div class="max-w-3xl bg-white border border-gray-200 shadow-sm rounded-2xl p-8">
        <form action="{{ route('admin.inventory.store') }}" method="POST" class="space-y-6 text-xs">
            @csrf

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="item_name" class="block font-bold text-gray-750 uppercase tracking-wide">Nama Barang / Aset</label>
                    <input type="text" id="item_name" name="item_name" required value="{{ old('item_name') }}" placeholder="Contoh: AC Standing 5 PK, Genset, Karpet Masjid" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-805">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="code" name="code" required value="{{ old('code') }}" placeholder="Contoh: AST-2026-AC01" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-805 font-mono">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="number" id="quantity" name="quantity" min="1" required value="{{ old('quantity', 1) }}" placeholder="1" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800 font-mono font-semibold">
                ...
                <div class="space-y-2">
                    ...
                    <input type="date" id="purchase_date" name="purchase_date" value="{{ old('purchase_date') }}" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input type="text" id="location" name="location" required value="{{ old('location') }}" placeholder="Contoh: Ruang Utama, Gudang Masjid" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-805">
                    <x-input-error :messages="$errors->get('location')" class="mt-1" />
                </div>

                <div class="space-y-2">
                    <label for="notes" class="block font-bold text-gray-750 uppercase tracking-wide">Catatan / Keterangan</label>
                    <textarea id="notes" name="notes" rows="3" placeholder="Informasi tambahan, merk, spesifikasi, dsb (Opsional)" class="w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-200 rounded-xl px-4 py-3 text-gray-800">{{ old('notes') }}</textarea>
